//! A windowing shell for Iced, on top of [`tao`].
use iced_debug::core::theme::Base;
    ...
    P::Theme: theme::Base,
{
    ...
    let mut event_loop_builder =
        EventLoopBuilder::<Action<P::Message>>::with_user_event();

    let event_loop = event_loop_builder.build();

    let (proxy, worker) = Proxy::<P::Message>::new(event_loop.create_proxy());
    ...
    struct Runner<Message: 'static, F> {
        ...
        id: Option<String>,
        sender: mpsc::UnboundedSender<Event<'static, Action<Message>>>,
        ...
        F: Future<Output = ()>,
    {
        fn process_event(
            ...
            event_loop: &tao::event_loop::EventLoopWindowTarget<
                Action<Message>,
            >,
            event: Event<'static, Action<Message>>,
            control_flow: &mut ControlFlow,
        ) {
            if let Event::EventLoopAwakened(tao::event::Event::LoopDestroyed) =
                event
            {
                return;
            }
    ...
    let _ = event_loop.run(move |event, event_loop, control_flow| {
        let event = event.to_static().expect("static event");
...
enum Event<'a, Message: 'static> {
    ...
    EventLoopAwakened(tao::event::Event<'a, Message>),
}

#[derive(Debug)]
enum Control {
    ChangeFlow(tao::event_loop::ControlFlow),
    Exit,
    Crash(Error),
    CreateWindow {
        id: window::Id,
    ...
    match action {
        ...
        Action::Window(action) => match action {
            ...
            window::Action::Move(id, position) => {
                if let Some(window) = window_manager.get_mut(id) {
                    window.raw.set_outer_position(tao::dpi::LogicalPosition {
                        x: position.x,
                        y: position.y,
                    });
                }
            ...
            window::Action::GetMode(id, channel) => {
                if let Some(window) = window_manager.get_mut(id) {
                    let mode = if window.raw.is_visible() {
            ...
            window::Action::EnableMousePassthrough(id) => {
                if let Some(window) = window_manager.get_mut(id) {
                    let _ = window.raw.set_ignore_cursor_events(true);
                }
            }
            window::Action::DisableMousePassthrough(id) => {
                if let Some(window) = window_manager.get_mut(id) {
                    let _ = window.raw.set_ignore_cursor_events(false);
                }
            }
        },
