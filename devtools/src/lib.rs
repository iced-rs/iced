            ...
            P: $Program + 'static,
        {
                ...
                match event {
                    Event::Message(message) => match message {
                        ...
                        Message::InstallComet => {
                            self.mode = Mode::Setup(Setup::Running {
                                logs: Vec::new(),
                            });
                        ...
                        Message::Installing(Ok(installation)) => {
                            let Mode::Setup(Setup::Running { logs }) =
                                &mut self.mode
                            else {
                                return Task::none();
                            };
                        ...
                        Message::Installing(Err(error)) => {
                            ...
                            match error {
                                comet::install::Error::ProcessFailed(
                                    status,
                                ) => {
                                    logs.push(format!(
            ...
            fn view(
                ...
                let mode = match &self.mode {
                    Mode::None => None,
                    Mode::Setup(setup) => {
                        let stage: Element<'_, _, Theme, P::Renderer> =
                            match setup {
                ...
                .map(|mode| {
                    themer(
                        derive_theme(),
                        Element::from(mode).map(Event::Message),
                    )
                });
