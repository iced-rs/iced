...
enum Message {
    ...
    GamepadDisconnected(gamepad::Id),
    GamepadButtonPressed(gamepad::Id, gamepad::Button),
    GamepadButtonChanged(gamepad::Id, gamepad::Button, f32),
    FocusGamepad(gamepad::Id),
    ThumbStickMoved(gamepad::Id, gamepad::Thumbstick, gamepad::Axis, f32),
    Discard,
}
...
impl App {
    ...
    fn update(&mut self, message: Message) {
        match message {
            ...
            Message::GamepadButtonPressed(id, button) => {
                self.update_button(id, button, 1.0);
            }
            Message::GamepadButtonChanged(id, button, value) => {
                self.update_button(id, button, value)
            }
            Message::ThumbStickMoved(id, thumbstick, axis, value) => {
    ...
    fn subscription(&self) -> Subscription<Message> {
        iced::gamepad::listen().map(|event| {
            use gamepad::Event;
            match event {
                Event::Connected(id) => Message::GamepadConnected(id),
                Event::Disconnected(id) => Message::GamepadDisconnected(id),
                Event::ButtonPressed { id, button, .. } => {
                    Message::GamepadButtonPressed(id, button)
                }
                Event::ButtonChanged { id, button, value } => {
                    Message::GamepadButtonChanged(id, button, value)
                }
                Event::AxisChanged {
                    id,
