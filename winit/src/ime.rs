...
use iced_runtime::{command, ime::Action, Command};
...
pub fn set_ime_allowed<Message>(allowed: bool) -> Command<Message> {
    Command::single(command::Action::IME(Action::Allow(allowed)))
}
...
pub fn set_position<Message>(x: i32, y: i32) -> Command<Message> {
    Command::single(command::Action::IME(Action::Position(x, y)))
}
